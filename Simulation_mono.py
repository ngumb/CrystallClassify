...
"""
Takes a large gold fcc structure object and then 
proceed to take cutouts from that structure

"""


ImgNum =  1 # Number of images to simulate
ht = 300
nx = 512
ny = 512
nz = 64
...
for j in range(ImgNum):
    ...
    NPsize =  0.5+np.random.uniform()*3.5
    
    RotM=R.from_euler("XYZ", [alpha,beta,gamma])
    
    FileName = 'MonoGold' + '_a{:.0f}'.format(alpha) + '_b{:.0f}'.format(beta) + '_c{:.0f}'.format(gamma) +  '_j{:.0f}'.format(j)
    ...
    #fcel.write_CEL(sc1, 'D:/04_Code/04_DrProbe/Au-NPs/Au-Icosahedron/cel/147i.cel')
    # Read cell file, rotate coordinates and then write back to cell file, also save as cif for viewing purpose
    ...
    # Save prm file
    msa.save_msa_prm(Maindir + "/prm/msa2.prm")
    drp.commands.msa(Maindir + "/prm/msa2.prm", Maindir + "/wav/"+ "Au2" +".dat", ctem=True, output=True )
    ...
    wavimg.wave_files = Maindir + "/wav/" + "Au2" + "_sl{:03d}.wav".format(sl)
    ...
    mpli.imsave(Maindir + "/img/" + FileName + ".png",img,cmap='gray')
